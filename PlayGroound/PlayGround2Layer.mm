...
-(void) createBullet:(ccTime)deltaTime withTarget:(CGPoint)bulletTarget withVelocity:(CGPoint)targetVelocity {
    ...
    if (weaponToFire == kWeaponBullets) {
        ...
        if ((missleCount < MISSILE_LIMIT) && (missleTime >= MISSLE_FIRE_DELAY)){
            ...
            fireMissle.body->SetLinearVelocity(rocket.body->GetWorldVector(b2Vec2(0,5.0f)));
            [sceneSpriteBatchNode addChild:fireMissle];
            [fireMissle release];
            missleCount ++;
            missleTime = 0.0;
        }
    } else if (weaponToFire == kWeaponMissiles) {
        if ((missleCount < MISSILE_LIMIT) && (missleTime >= MISSLE_FIRE_DELAY)){
            ...
            [fireMissle setDelegate:self];
            fireMissle.body->SetLinearVelocity(rocket.body->GetWorldVector(b2Vec2(0,1.0f)));
            [sceneSpriteBatchNode addChild:fireMissle];
            [fireMissle release];
            missleCount ++;
            missleTime = 0.0;
